...
Standardized message shapes. Three tiers, each with a distinct visual
signature so the recipient can identify message type before reading content.

Tier 1 — Chat:     No formatting. Natural conversation.
                    Handled by the agent's chat loop, not this module.

Tier 2 — Report:   Structured, scannable. Daily briefs, summaries,
                    health digests. Emoji header + bullet structure.

Tier 3 — Alert:    Urgent interrupt. Critical failures, high-severity
                    triggers. Bold title + error context + action line.
...
def fmt_report(
    ...
    Args:
        title: Report header (e.g. "Morning Brief — Apr 10").
...
def fmt_job_health(
    ...
    """Format overnight job health for a morning brief.
...
def should_interrupt(severity: Severity) -> bool:
    """Should this severity interrupt the operator via Telegram immediately?
    ...
    if severity is Severity.HIGH:
        # Check waking hours (CT = UTC-5 / UTC-6 depending on DST)
